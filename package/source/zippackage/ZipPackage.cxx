				...
				if ( xMetaInfFolder.is() && xMetaInfFolder->hasByName( sManifest ) )
				{
					...
					if (xSink.is())
					{
						...
						if ( xReader.is() )
						{
							...
							const OUString sPropDigestType ( RTL_CONSTASCII_USTRINGPARAM ( "DigestType" ) );
							const OUString sPropAlgorithm ( RTL_CONSTASCII_USTRINGPARAM ( "Algorithm" ) );
							...
							for (sal_Int32 i = 0; i < nLength ; i++, pSequence++)
							{
...
#ifndef NO_OOO_3_4_1_AES_ENCRYPTION
								const Any *pDigestType = NULL, *pAlgorithm = NULL, *pDerivedKeySize = NULL, *pStartKeyAlgorithm = NULL;
#endif	// !NO_OOO_3_4_1_AES_ENCRYPTION
								for (sal_Int32 j = 0, nNum = pSequence->getLength(); j < nNum; j++ )
								{
									if (pValue[j].Name.equals( sPropFullPath ) )
...
#ifndef NO_OOO_3_4_1_AES_ENCRYPTION
									else if (pValue[j].Name.equals( sPropDigestType ) )
										pDigestType = &(pValue[j].Value);
									else if (pValue[j].Name.equals( sPropAlgorithm ) )
										pAlgorithm = &(pValue[j].Value);
										...
										pStartKeyAlgorithm = &(pValue[j].Value);
#endif  // !NO_OOO_3_4_1_AES_ENCRYPTION
								}
											...
											*pSize >>= nSize;
											...
											if ( pDigest )
											{
												*pDigest >>= aSequence;
												...
												if ( pDigestType )
												{
													sal_Int32 nDigestType = 0;
													*pDigestType >>= nDigestType;
													pStream->setDigestType( nDigestType );
												}
#endif  // !NO_OOO_3_4_1_AES_ENCRYPTION
											}
											...
											if ( pAlgorithm )
											{
												...
												*pAlgorithm >>= nAlgorithm;
												pStream->setAlgorithm( nAlgorithm );
											}
											...
											if ( pDerivedKeySize )
											{
												...
												*pDerivedKeySize >>= nDerivedKeySize;
												pStream->setDerivedKeySize( nDerivedKeySize );
											}

											if ( pStartKeyAlgorithm )
											{
