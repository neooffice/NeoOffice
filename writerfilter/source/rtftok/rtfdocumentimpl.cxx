...
{
#if SUPD == 310
    return util::DateTime2(0 /*100sec*/, 0 /*sec*/, aState.nMinute, aState.nHour,
                          aState.nDay, aState.nMonth, aState.nYear, 0, false);
#else	// SUPD == 310
...
#if SUPD == 310
        uno::Reference<graphic::XGraphicProvider> xGraphicProvider(m_xContext->getServiceManager()->createInstanceWithContext(OUString(RTL_CONSTASCII_USTRINGPARAM("com.sun.star.graphic.GraphicProvider")), m_xContext), uno::UNO_QUERY_THROW);
#else	// SUPD == 310
...
#if SUPD == 310
        m_pBinaryData->Write(&ch, sizeof(ch));
#else	// SUPD == 310
...
#if SUPD == 310
            if (OString(aBuf.getStr()).indexOf(OString("FORM")) != -1)
#else	// SUPD == 310
...
int RTFDocumentImpl::dispatchValue(RTFKeyword nKeyword, int nParam)
{
    ...
    {
#if SUPD == 310
        RTFValue::Pointer_t pValue(new RTFValue(MsLangId::convertLanguageToIsoString((LanguageType)nParam)));
#else	// SUPD == 310
...
int RTFDocumentImpl::popState()
{
...
#if SUPD == 310
        String replacedData(str);
        replacedData.SearchAndReplaceAll(String("\""), String("\\\""));
        str = field + " \"" + (OUString)replacedData + "\"";
#else   // SUPD == 310
    ...
    case DESTINATION_OBJDATA:
    {
...
#if SUPD == 310
                    char c = (char)b;
                    m_pObjectData->Write(&c, sizeof(c));
#else	// SUPD == 310
        ...
        if (m_pObjectData->Tell())
        {
            ...
            sal_uInt32 nData;
#if SUPD == 310
            m_pObjectData->ReadNumber(nData);   // OLEVersion
            m_pObjectData->ReadNumber(nData);   // FormatID
            m_pObjectData->ReadNumber(nData);   // ClassName
#else	// SUPD == 310
...
#if SUPD == 310
            m_pObjectData->ReadNumber(nData);   // TopicName
#else	// SUPD == 310
            m_pObjectData->ReadUInt32(nData);   // TopicName
#endif	// SUPD == 310
            m_pObjectData->SeekRel(nData);
#if SUPD == 310
            m_pObjectData->ReadNumber(nData);   // ItemName
#else	// SUPD == 310
            m_pObjectData->ReadUInt32(nData);   // ItemName
#endif	// SUPD == 310
            m_pObjectData->SeekRel(nData);
#if SUPD == 310
            m_pObjectData->ReadNumber(nData);   // NativeDataSize
#else	// SUPD == 310
...
{
#if SUPD == 310
    if (bUnicode && m_aUnicodeBuffer.getLength())
#else	// SUPD == 310
...
#if SUPD == 310
    if (bHex && m_aHexBuffer.getLength())
#else	// SUPD == 310
