...
{
    MenuItemData* pData = pItemList->GetDataFromPos( (sal_uLong)nPos );
    if ( pData )
        ...
        if ( ImplIsVisible( n ) && ( !pThisItemOnly || ( pData == pThisItemOnly ) ) )
        {
...
#ifdef USE_JAVA
            {
                if ( pWin->IsNativeControlSupported( CTRL_MENU_POPUP, PART_ENTIRE_CONTROL ) )
                {
                    ...
                    Color aTextColor;
                    if ( pWin->GetNativeControlTextColor( CTRL_MENU_POPUP, PART_ENTIRE_CONTROL, CTRL_STATE_SELECTED, aValue, aTextColor ) )
