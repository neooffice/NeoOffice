...
#if defined USE_JAVA && defined MACOSX
	// Fix bug 613 by checking if the front window is a native modal window
	// as we will deadlock when a native modal window is showing
	WindowRef aWindow = FrontWindow();
	WindowModality nModality;
	if ( aWindow && GetWindowModality( aWindow, &nModality, NULL ) == noErr && ( nModality == kWindowModalitySystemModal || nModality == kWindowModalityAppModal ) )
		return 0;
#endif	// USE_JAVA && MACOSX
