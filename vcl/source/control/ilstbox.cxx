...
#ifdef USE_JAVA
                bNativeOK = pWin->DrawNativeControl( CTRL_LISTBOX, PART_ENTIRE_CONTROL, aParentRect,
#else	// USE_JAVA
	...
	if( !IsInPopupMode() )
	{
		...
		if ( GetParent()->IsNativeWidgetEnabled() )
		{
			if( dynamic_cast<ComboBox *>(GetParent()) && IsNativeControlSupported( CTRL_COMBOBOX, PART_ENTIRE_CONTROL ) )
			{
				...
				if ( GetNativeControlRegion( CTRL_COMBOBOX, PART_ENTIRE_CONTROL, aCtrlRegion, 0, aControlValue, rtl::OUString(), aBoundingRgn, aContentRgn ) )
			...
			if( dynamic_cast<ListBox *>(GetParent()) && IsNativeControlSupported( CTRL_LISTBOX, PART_ENTIRE_CONTROL ) )
			{
				...
				Rectangle aCtrlRegion( GetParent()->GetPosPixel(), GetParent()->GetSizePixel() );
				Rectangle aBoundingRgn, aContentRgn;
				if ( GetNativeControlRegion( CTRL_LISTBOX, PART_ENTIRE_CONTROL, aCtrlRegion, 0, aControlValue, rtl::OUString(), aBoundingRgn, aContentRgn ) )
					aSz = aContentRgn.GetSize();
			}
		}
#endif	// USE_JAVA
