 ...
 *
 * Modified February 2009 by Patrick Luby. NeoOffice is distributed under
...
#ifdef USE_JAVA
    // Fix bug 3418 by handling the mismatched resource array for non-English
    // languages
    if ( rUnit == FUNIT_CHAR )
        return String::CreateFromAscii( "ch" );
    else if ( rUnit == FUNIT_LINE )
        return String::CreateFromAscii( "line" );
    else if ( rUnit == FUNIT_PERCENT && strAllUnits->Count() )
        return strAllUnits->GetString( strAllUnits->Count() - 1 );
#endif	// USE_JAVA
