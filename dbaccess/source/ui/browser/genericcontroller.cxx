 ...
 *
 * Modified December 2015 by Patrick Luby. NeoOffice is distributed under
...
Reference< XFrame > SAL_CALL OGenericUnoController::getFrame(void) throw( RuntimeException )
{
    ...
    // separate thread. In the ODocumentDefinition::onCommandOpenSomething()
    // method, the call to OCommonEmbeddedObject::changeState() will release
    // the application mutex so we release the application mutex here so that
    // the other thread can reacquire it and then release this object's mutex.
