...
sal_Bool SAL_CALL ODatabaseDocument::isReadonly(  ) throw (RuntimeException)
{
    ...
    // separate thread. The OGenericUnoController::getFrame() method will
    // release the application mutex so we release the application mutex here
    // so that the other thread can reacquire it and then release this object's
    // mutex.
