...
static sal_Bool bForceSysAlloc = sal_True;
static sal_Bool bForceSysAllocInited = sal_False;

static sal_Bool rtl_force_sysalloc()
{
	if ( !bForceSysAllocInited )
	{
 		if ( getenv( "SAL_NO_FORCE_SYSALLOC" ) != NULL )
			bForceSysAlloc = sal_False;
		bForceSysAllocInited = sal_True;
	}

	return bForceSysAlloc;
}
