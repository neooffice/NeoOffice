...
{
#ifdef USE_JAVA
    sal_Bool bUserTempDirFound = sal_False;
    rtl_uString *pBrandBaseDir = NULL;
    rtl_uString *pBootstrapURL = NULL;
    rtl_uString_newFromAscii( &pBrandBaseDir, "BRAND_BASE_DIR" );
    if ( rtl_bootstrap_get( pBrandBaseDir, &pBootstrapURL, NULL ) )
    {
        rtl_uString *pBootstrapSubPath = NULL;
        rtl_uString_newFromAscii( &pBootstrapSubPath, "/program/" SAL_CONFIGFILE( "bootstrap" ) );
        rtl_uString_newReplaceStrAt( &pBootstrapURL, pBootstrapURL, rtl_uString_getLength( pBootstrapURL ), 0, pBootstrapSubPath );
        rtl_uString_release( pBootstrapSubPath );

        rtlBootstrapHandle aBootstrapHandle = rtl_bootstrap_args_open( pBootstrapURL );
        if ( aBootstrapHandle )
        {
            rtl_uString *pUserDir = NULL;
            rtl_uString *pUserInstallation = NULL;
            rtl_uString_newFromAscii( &pUserInstallation, "UserInstallation" );
            if ( rtl_bootstrap_get_from_handle( aBootstrapHandle, pUserInstallation, &pUserDir, NULL ) )
            {
                rtl_uString *pTmpSubDir = NULL;
                rtl_uString_newFromAscii( &pTmpSubDir, "/user/temp" );
                rtl_uString_newReplaceStrAt( pustrTempDir, pUserDir, rtl_uString_getLength( pUserDir ), 0, pTmpSubDir );
                rtl_uString_release( pTmpSubDir );

                bUserTempDirFound = sal_True;
                rtl_uString_release( pUserDir );
            }

            rtl_bootstrap_args_close( aBootstrapHandle );
            rtl_uString_release( pUserInstallation);
        }

        rtl_uString_release( pBootstrapURL );
    }
