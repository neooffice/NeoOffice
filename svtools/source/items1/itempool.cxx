...
#ifdef USE_JAVA
		// Partial fix for bug 3512 by not deleting items too early
		if ( !ReleaseRef( **( pDefaults + n ) ) )
#else	// USE_JAVA
			...
			if ( *ppStaticDefaultItem && (*ppStaticDefaultItem)->ISA(SfxSetItem) )
			{
				if ( *ppItemArr )
				{
					...
					for ( USHORT n = (*ppItemArr)->Count(); n; --n, ++ppHtArr )
						...
						{
#ifdef USE_JAVA
							// Partial fix for bug 3512 by not deleting items
							// too early
							if ( !ReleaseRef( **ppHtArr ) )
								delete *ppHtArr; *ppHtArr = 0;
#else	// USE_JAVA
#ifndef PRODUCT
...
#ifdef USE_JAVA
			// Partial fix for bug 3512 by not deleting items too early
			if ( !ReleaseRef( **ppDefaultItem ) )
				delete *ppDefaultItem; *ppDefaultItem = 0;
#else	// USE_JAVA
#ifndef PRODUCT
			SetRefCount( **ppDefaultItem, 0 );
#endif
