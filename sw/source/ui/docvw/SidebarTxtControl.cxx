    ...
    if ( rCEvt.GetCommand() == COMMAND_CONTEXTMENU )
    {
        ...
        else
        {
            ...
            if ( pOutlinerView )
            {
                ...
                ((PopupMenu *)pMgr->GetSVMenu())->InsertItem( FN_LOOKUP_IN_MACOSX_DICTIONARY, GetMacDictLoookupResString( STR_LOOKUP_IN_MACOSX_DICTIONARY ), 0, 0 );
                ((PopupMenu *)pMgr->GetSVMenu())->EnableItem( FN_LOOKUP_IN_MACOSX_DICTIONARY, sal_False );

                ESelection aSel( pOutlinerView->GetEditView().GetSelection() );
                String aText( pOutlinerView->GetEditView().GetEditEngine()->GetWord( aSel.nStartPara, aSel.nStartPos ) );
                if ( aText.Len() )
                    ((PopupMenu *)pMgr->GetSVMenu())->EnableItem( FN_LOOKUP_IN_MACOSX_DICTIONARY, sal_True );
            }
#endif	// USE_JAVA && MACOSX
