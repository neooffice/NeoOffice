...
#ifdef USE_JAVA
    // Fix bug 3624 by retrying with the first hash set to SHA1 when both
    // hashes are unspecified
    if (aHash != maPassHash && meHash1 == PASSHASH_UNSPECIFIED && meHash2 == PASSHASH_UNSPECIFIED)
    {
        aHash = hashPassword(aPassText, PASSHASH_SHA1);
        aHash = hashPassword(aHash, PASSHASH_UNSPECIFIED);
    }
#endif	// USE_JAVA
