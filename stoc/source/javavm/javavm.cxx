...
#ifdef MACOSX
#define DEF_JAVALIB "/System/Library/Frameworks/JavaVM.framework/JavaVM"
#else
...
static void getINetPropsFromConfig(JVM * pjvm,
	...
	class OCreatorThread : public Thread {
		friend class JavaVirtualMachine_Impl;
...
JavaVM * JavaVirtualMachine_Impl::createJavaVM(const JVM & jvm) throw(RuntimeException) {
...
#ifdef MACOSX
	sal_uInt16 stdprops= 3;
#else
	sal_uInt16 stdprops= 1;
#endif
	JavaVMOption * options= new JavaVMOption[cprops + stdprops];
	OString sClassPath= OString("-Djava.class.path=") + vm_args.classpath;
	options[0].optionString= (char*)sClassPath.getStr();
	options[0].extraInfo= NULL;
#ifdef MACOSX
	options[1].optionString= (char*)OString("-Dcom.apple.hwaccel=false").getStr();                                     
	options[1].extraInfo= NULL;
	options[2].optionString= (char*)OString("-Dcom.apple.hwaccellist=").getStr();                                     
	...
	{				
		...
		options[x+stdprops].optionString= (char*)arProps[x].getStr();
		options[x+stdprops].extraInfo= NULL;
	}
	vm_args2.options= options;
	vm_args2.nOptions= cprops + stdprops;
	vm_args2.ignoreUnrecognized= JNI_TRUE;

	// Try 1.4 first since some JVMs will reduce the features if we ask
	// for 1.2
	vm_args2.version= 0x00010004;
	err= pCreateJavaVM(&pJavaVM, &pJNIEnv, &vm_args2);

	// If we can't load 1.4, try 1.2
	if (err)
	{
