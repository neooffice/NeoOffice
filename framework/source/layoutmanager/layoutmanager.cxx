        ...
        try
        {
            ...
            if ( xUIElement.is() )
            {
...
#ifdef USE_JAVA
                bool bUseGenericAddonTitle = false;
                if ( aAddonToolBarData.getLength() )
                {
                    Sequence< PropertyValue > aFirstButtonData = aAddonToolBarData[ 0 ];
                    sal_uInt32 nButtonCount = aFirstButtonData.getLength();
                    for ( sal_uInt32 j = 0; j < nButtonCount; j++ )
                    {
                        if ( aFirstButtonData[ j ].Name == ADDONSMENUITEM_PROPERTYNAME_TITLE )
                        {
                            ...
                            if ( aTitle.getLength() )
                            {
                                String aNewTitle = String( FwkResId( STR_TOOLBAR_TITLE_ADDON ) );
                                aTitle = OUString( RTL_CONSTASCII_USTRINGPARAM( "- " ) ) + aTitle;
                                aNewTitle.SearchAndReplaceAscii( "%num%", aTitle );
                                if ( aNewTitle.Len() )
                                {
                                    bUseGenericAddonTitle = true;
                                    aGenericAddonTitle = OUString( aNewTitle );
                                }
...
#ifdef USE_JAVA
                    if ( rElement.m_aUIName.getLength() == 0 || bUseGenericAddonTitle )
#else	// USE_JAVA
...
#ifdef USE_JAVA
                    if ( aNewToolbar.m_aUIName.getLength() == 0 || bUseGenericAddonTitle )
#else	// USE_JAVA
                ...
                if ( xWindow.is() )
                {
                    ...
                    Window* pWindow = VCLUnoHelper::GetWindow( xWindow );
#ifdef USE_JAVA
                    if ( pWindow->GetText().Len() == 0 || bUseGenericAddonTitle )
#else	// USE_JAVA
...
#ifdef USE_JAVA
    // Fix bug 3045 by checking that the element is valid
    if ( aUIDockingElement.m_xUIElement.is() && rUIElement.m_aName == aUIDockingElement.m_aName )
#else	// USE_JAVA
