...
sal_Bool SAL_CALL Desktop::terminate() throw( css::uno::RuntimeException )
{
    ...
    else
    {
        ...
        // If all tasks have been disposed but we are not terminating, create
		// a new task with a backing window
        css::uno::Reference< css::container::XElementAccess > xList( getFrames(), css::uno::UNO_QUERY );
fprintf( stderr, "Here: %i %i\n", xList.is(), xList.is() ? xList->hasElements() : 0 );
        if ( !xList.is() || !xList->hasElements() )
        {
            css::uno::Reference< css::frame::XFrame > xFrame = findFrame( SPECIALTARGET_BLANK, 0 );
             css::uno::Reference< css::awt::XWindow > xContainerWindow = xFrame->getContainerWindow();
             css::uno::Sequence< css::uno::Any > lArgs(1);
             lArgs[0] <<= xContainerWindow;

             css::uno::Reference< css::frame::XController > xBackingComp( m_xFactory->createInstanceWithArguments( SERVICENAME_STARTMODULE, lArgs ), css::uno::UNO_QUERY_THROW );
