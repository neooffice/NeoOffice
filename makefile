...
# Regroup the OOo langauge packs
	...
	sh -e -c 'for i in `cat etc/supportedlanguages.txt | awk -F, "{ print \\$$1 }"` ; do if [ -d "$(BUILD_HOME)/helpcontent2/$(UOUTPUTDIR)/misc/$${i}/text" ] ; then echo "$${i}" ; fi ; done' > "$(PWD)/$(INSTALL_HOME)/language_names"
	rm -Rf "$(INSTALL_HOME)/package/Applications"
# Create the language pack installers
	sh -e -c 'for i in `cat "$(PWD)/$(INSTALL_HOME)/language_names"` ; do if [ "$${i}" = "en-US" ] ; then continue ; fi ; langname=`grep "^$${i}," "$(PWD)/etc/supportedlanguages.txt" | awk -F, "{ print \\$$2 }" | sed "s# #_#g"` ; if [ -z "$${langname}" ] ; then exit 1 ; fi ; mkdir -p "$(PWD)/$(INSTALL_HOME)/package_$${langname}/Contents" ; mkdir -p "$(PWD)/$(INSTALL_HOME)/package_$${langname}/Applications" ; ln -sf "$(PWD)/$(INSTALL_HOME)/package_$${langname}/Contents" "$(PWD)/$(INSTALL_HOME)/package_$${langname}/Applications/$(OO_DIR_NAME)" ; ( cd "$(PWD)/$(INSTALL_HOME)/package_$${langname}" ; for j in `find "$(PWD)/$(BUILD_HOME)/instsetoo_native/$(UOUTPUTDIR)/OpenOffice_languagepack/portable/install/$${i}" -name "*.sw"` ; do gnutar xvf "$${j}" ; done ; for j in `find "$(PWD)/$(BUILD_HOME)/instsetoo_native/$(UOUTPUTDIR)/OpenOffice_languagepack/portable/install/$${i}" -name "*.install"` ; do grep "mkdir " "$${j}" | grep "/Applications" | sed "s#^.*/Applications#Applications#" | xargs -n1 mkdir -p ; done ) ; rm -Rf "$(PWD)/$(INSTALL_HOME)/package_$${langname}/Applications" ; "$(MAKE)" $(MFLAGS) "build.package_$${langname}" ; done'
	...
	cd "$(INSTALL_HOME)/package/Contents" ; cp "$(PWD)/$(BUILD_HOME)/desktop/$(UOUTPUTDIR)/bin/soffice" "program/soffice.bin" ; chmod a+x "program/soffice.bin"
	...
	cd "$(INSTALL_HOME)/package/Contents" ; cp "$(PWD)/$(BUILD_HOME)/vcl/$(UOUTPUTDIR)/bin/salapp"*.res "program/resource"
	...
	cd "$(INSTALL_HOME)/package/Contents" ; sed 's#$$(OO_PRODUCT_NAME)#$(PRODUCT_NAME)#g' "$(PWD)/etc/help/main_transform.xsl" | sed 's#$$(OO_SUPPORT_URL)#$(PRODUCT_SUPPORT_URL)#g' | sed 's#$$(OO_SUPPORT_URL_TEXT)#$(PRODUCT_SUPPORT_URL_TEXT)#g' | sed 's#$$(PRODUCT_SUPPORT_URL)#$(PRODUCT_SUPPORT_URL)#g' | sed 's#$$(PRODUCT_SUPPORT_URL_TEXT)#$(PRODUCT_SUPPORT_URL_TEXT)#g' > "help/main_transform.xsl"
