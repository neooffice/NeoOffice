...
NEO_TAG:=NeoOfficeJ-1_1_Alpha_2
...
build.package: build.neo_patches build.oo_download_dics build.source_zip
	...
	echo `source "$(OO_ENV_JAVA)" ; cd "instsetoo/util" ; dmake give_me_all_languages=TRUE language_names ; awk '{ print $$3 "-" $$4 }' "$(PWD)/$(DIC_HOME)/dictionary.lst"` | sort -u > "$(INSTALL_HOME)/all_language_names"
...
# Eliminate duplicate help directories since only English is available
	...
	cd "$(INSTALL_HOME)/package/Contents/help" ; sh -e -c 'for i in `cat "$(PWD)/$(INSTALL_HOME)/all_language_names"` ; do ln -sf "en" "$${i}" ; done'
	...
	cd "$(INSTALL_HOME)/package/Contents" ; sed 's#$$(PRODUCT_DIR_NAME)#$(PRODUCT_DIR_NAME)#g' "$(PWD)/$(BUILD_HOME)/setup2/unxmacxp.pro/misc/setup.sh" | sed 's#$$(OO_VERSION)#$(OO_VERSION)#g' | sed 's#$$(LANGUAGE_NAMES)#'"`cat "$(PWD)/$(INSTALL_HOME)/language_names"`"'#g' > "program/setup" ; chmod a+x "program/setup"
	...
	cd "$(INSTALL_HOME)/package/Contents/Resources" ; sh -e -c 'for i in `cat "$(PWD)/$(INSTALL_HOME)/all_language_names" | sed "s#-#_#g"` ; do mkdir -p "$${i}.lproj" ; mkdir -p `echo "$${i}" | sed "s#_.*\\$$##"`".lproj" ;  done'
	...
	mkdir -p "$(INSTALL_HOME)/$(PRODUCT_DIR_NAME).pkg/Contents/Resources/en.lproj"
	...
	sed 's#$$(PRODUCT_NAME)#$(PRODUCT_NAME)#g' "etc/neojava.info" | sed 's#$$(PRODUCT_DIR_NAME)#$(PRODUCT_DIR_NAME)#g' | sed 's#$$(PRODUCT_VERSION)#$(PRODUCT_VERSION)#g' > "$(INSTALL_HOME)/$(PRODUCT_DIR_NAME).pkg/Contents/Resources/en.lproj/$(PRODUCT_DIR_NAME).info"
...
build.patch_package: build.package
	...
	cp "$(INSTALL_HOME)/package/Contents/help/main_transform.xsl" "$(PATCH_INSTALL_HOME)/package/Contents/help/main_transform.xsl"
	...
	cd "$(PATCH_INSTALL_HOME)/package/Contents" ; sed 's#$$(PRODUCT_DIR_NAME)#$(PRODUCT_DIR_NAME)#g' "$(PWD)/$(BUILD_HOME)/setup2/unxmacxp.pro/misc/setup.sh" | sed 's#$$(OO_VERSION)#$(OO_VERSION)#g' | sed 's#$$(LANGUAGE_NAMES)#'"`cat "$(PWD)/$(INSTALL_HOME)/language_names"`"'#g' > "program/setup" ; chmod a+x "program/setup"
	...
	mkdir -p "$(PATCH_INSTALL_HOME)/$(PRODUCT_DIR_NAME).pkg/Contents/Resources/en.lproj"
	...
	sed 's#$$(PRODUCT_NAME)#$(PRODUCT_NAME)#g' "etc/neojava.info.patch" | sed 's#$$(PRODUCT_DIR_NAME)#$(PRODUCT_DIR_NAME)#g' | sed 's#$$(PRODUCT_VERSION)#$(PRODUCT_VERSION)#g' | sed 's#$$(PRODUCT_PATCH_VERSION)#$(PRODUCT_PATCH_VERSION)#g' > "$(PATCH_INSTALL_HOME)/$(PRODUCT_DIR_NAME).pkg/Contents/Resources/en.lproj/$(PRODUCT_DIR_NAME).info"
	...
	cp "bin/VolumeCheck.strings.patch" "$(PATCH_INSTALL_HOME)/$(PRODUCT_DIR_NAME).pkg/Contents/Resources/en.lproj/VolumeCheck.strings"
# Mac OS X 10.2.8 cannot handle a postflight script
