...
# increasing versions.
	...
	cd "$(INSTALL_HOME)/package/Contents" ; sh -e -c 'for i in `find "share/config/soffice.cfg/modules" -name "menubar.xml"` ; do sed "s#<menu:menuitem.*\.uno:TwainSelect.*/>#<\!--&-->#g" "$${i}" > "../../out" ; mv -f "../../out" "$${i}" ; done'
...
# Reregister vcl to enable custom drag and drop service
	source "$(OO_ENV_JAVA)" ; setenv DYLD_LIBRARY_PATH "$(PWD)/$(INSTALL_HOME)/package/Contents/program:$$DYLD_LIBRARY_PATH" ; cd "$(INSTALL_HOME)/package/Contents/program" ; "$(PWD)/$(INSTALL_HOME)/package/Contents/program/regcomp" -revoke -r services.rdb -c 'vnd.sun.star.expand:$$OOO_BASE_DIR/program/libvcl.dylib'
	source "$(OO_ENV_JAVA)" ; setenv DYLD_LIBRARY_PATH "$(PWD)/$(INSTALL_HOME)/package/Contents/program:$$DYLD_LIBRARY_PATH" ; cd "$(INSTALL_HOME)/package/Contents/program" ; "$(PWD)/$(INSTALL_HOME)/package/Contents/program/regcomp" -register -r services.rdb -c 'vnd.sun.star.expand:$$OOO_BASE_DIR/program/libvcl.dylib'
