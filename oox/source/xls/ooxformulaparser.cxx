...
Sequence< FormulaToken > SAL_CALL OOXMLFormulaParser::parseFormula(
        const OUString& rFormula ) throw( RuntimeException )
{
    if( !mxParserImpl )
    {
    ...
    return mxParserImpl->parseFormula( rFormula, CellAddress( 0, 0, 0 ) );
}

OUString SAL_CALL OOXMLFormulaParser::printFormula(
        const Sequence< FormulaToken >& /*rTokens*/ ) throw( RuntimeException )
{
    // not implemented
    throw RuntimeException();
}
