...
, mbImpress( bImpress )
{
	if( bImpress )
	{
		...
		// Set the title to the first fixed line control's text
		// same control and hide that control
		String aTitle( mpControls[ FL_PRINTCONTENT - 1 ]->GetText() );
		aTitle.EraseAllChars( '~' );
		SetText( aTitle );
		mpControls[ FL_PRINTCONTENT - 1 ]->Hide();

		sal_Int32 nHeight = 0;

		sal_Int32 nInsertY = 0;
		...
		while( nChildCount-- )
		{
			...
			if( pControl )
			{
				...
				if ( bHideControls )
				{
					if ( dynamic_cast< FixedLine* >( pControl ) && !pControl->GetText().Len() && --nFixedLine == 0 )
						bHideControls = false;
					else
						pControl->Hide();
				}
