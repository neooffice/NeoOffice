...
void SfxApplication::NewDocExec_Impl( SfxRequest& rReq )
{
    ...
    if ( !pTemplNameItem && !pTemplFileNameItem )
    {
        ...
        // check the referrer. If it indicates an external using (e.g. the command line)
        // use the dialog in a special no parent mode!. Otherwise use it modal to the
        // DefModalDialogParanet - which is the most active task window.
        SFX_REQUEST_ARG(rReq, pReferrer, SfxStringItem, SID_REFERER, FALSE);
        String aReferrer;
        if (pReferrer)
            aReferrer = pReferrer->GetValue();

        if (aReferrer.Equals(String::CreateFromAscii("private:OpenEvent")))
            pTopWin = DIALOG_NO_PARENT;
        ...
        if( pLinkItem || rReq.IsSynchronCall() )
        {
            ...
            if ( xDisp.is() )
            {
                ...
                if ( xNot.is() )
                {
                    ...
                    {
                        pListener->bSuccess = FALSE;
                        pListener->bFinished = TRUE;
                        pListener->release();
                        return;
                    }
