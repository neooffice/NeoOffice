...
#include "dialog.hrc"
#ifndef _SVTOOLS_SVPATCH_HRC
#include <svtools/svpatch.hrc>
#endif
...
#ifdef MACOSX
static short gNumAboutDialogs=0;
#endif
...

{
	...
	// pb: c01 special
	if ( SvtHelpOptions().IsHelpOnStartupAvailable() )
		aCopyrightText.SetText( String( SvpResId( PID_STR_ABOUT_COPYRIGHT ) ) );
...
#ifdef MACOSX
    // count of the number of active instances to avoid creating duplicates
    gNumAboutDialogs++;
#endif
}
...
#ifdef MACOSX
/**
 * Determine if an about dialog is already being displayed by the application.
 * We can only display one modal instance of the about dialog at a time.
 *
 * @return TRUE if other instances exist, FALSE if no instance is available
 */
BOOL AboutDialog::DialogIsActive()
{
    return ( gNumAboutDialogs > 0 );
}
#endif